...
export function PostsSection() {
  ...
  const postsData = searchInput
    ? posts.filter((post) => post.post_text.toLowerCase().includes(searchInput.toLowerCase()))
    : allPosts

  const commentsData = searchInput
    ? posts.flatMap((post) =>
        post.comments.filter((comment) => comment.comment_text.toLowerCase().includes(searchInput.toLowerCase())),
      )
    : []
  ...
  return (
    <main className={styles["container"]}>
      <NewPost />
      <Link className={styles["filter"]} href={isFilterSelected ? "/posts/all" : "/posts/following"}>
